...
/**
 * In-app list panel for one curated DocType (no ERPNext desk iframe).
 */
export function CrmEntityList({
  doctype,
  apiBase,
  getAccessToken,
  title,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTotal(
        typeof countRes?.data === "number" ? countRes.data : (listRes.data ?? []).length,
      );
    } catch (e) {
  ...
  return (
    <Card title={heading}>
      <Typography.Paragraph type="secondary" className="!mb-3">
        ...
        (same permissions as your ERPNext user).
