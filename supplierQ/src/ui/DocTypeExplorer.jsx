...
export function DocTypeExplorer({ apiBase, getAccessToken }) {
  ...
  const [doctype, setDoctype] = useState("Request for Quotation");
  ...
  const loadDocTypes = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof GatewayErpNextError ? `${e.message} (${e.status})` : String(e));
    }
  }, [client]);
  ...
  return (
    <Card title="Generic DocType list">
      ...
      <Space wrap className="mb-4">
        <Select
          ...
          value={doctype}
          onChange={setDoctype}
          options={doctypes.map((d) => ({ value: d.name, label: `${d.name} (${d.module || "?"})` }))}
        />
        <Button type="primary" onClick={loadRows} loading={loading}>Load rows</Button>
        <Button onClick={loadDocTypes}>Refresh DocTypes</Button>
      </Space>
      {count !== null ? (
        <Typography.Text type="secondary" className="block mb-2">Count: {String(count)}</Typography.Text>
      ) : null}
      <Table
        size="small"
        ...
        loading={loading}
        dataSource={rows}
        columns={[{ title: "name", dataIndex: "name" }, { title: "modified", dataIndex: "modified" }]}
        pagination={false}
      />
    </Card>
  );
