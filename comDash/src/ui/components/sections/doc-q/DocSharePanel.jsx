...
import { Avatar, Button, Drawer, Empty, Form, List, Popconfirm, Select, Space, Tag, Typography, message } from "antd";
...
export default function DocSharePanel({ items = [], open, onClose, onChanged }) {
  ...
  useEffect(() => {
    if (!open) return;
    load();
    apiFetch(`${docPath("/org/users")}?limit=100`).then(r => r.json()).then(j => setUsers(j.users || [])).catch(() => {});
  }, [open, load]);
  ...
  async function addShare(values) {
    ...
    try {
      ...
      message.success(`Successfully shared ${successCount} item(s)`);
      form.resetFields();
      await load();
      onChanged?.();
  ...
  return (
    <Drawer title={isSingleItem ? "Share" : `Share ${items.length} items`} placement="right" width={420} open={open} onClose={onClose} destroyOnClose>
      
      <Form form={form} layout="vertical" onFinish={addShare} initialValues={{ permission: "read" }}>
        <Form.Item name="granteeEmail" label="Add people" rules={[{ required: true, message: "Pick an email" }]}>
          <DocEmailSelect initialUsers={users} placeholder="Add people by email" style={{ width: "100%" }} />
        </Form.Item>
        <Space align="start" style={{ width: "100%", justifyContent: "space-between" }}>
          <Form.Item name="permission" label="Access" style={{ marginBottom: 0 }}>
            <Select style={{ width: 150 }} options={[{ value: "read", label: "Viewer" }, { value: "write", label: "Editor" }, { value: "approve", label: "Approver" }]} />
          </Form.Item>
          <Button type="primary" htmlType="submit" loading={adding} style={{ marginTop: 30 }}>Share</Button>
        </Space>
      </Form>

      {isSingleItem && (
        <>
          <Typography.Title level={5} style={{ marginTop: 24 }}>People with access</Typography.Title>
          {!loading && !shares.length ? ( <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Only you have access" /> ) : (
            <List loading={loading} dataSource={shares} rowKey="id" renderItem={(s) => {
                const perm = PERMISSION_LABEL[s.permission] || { label: s.permission, color: "default" };
                const who = s.grantee_email || "—";
                return (
                  <List.Item actions={[
                      <Popconfirm key="remove" title="Remove access?" onConfirm={() => removeShare(s.id)}>
                        <Button type="text" danger size="small">Remove</Button>
                      </Popconfirm> ]}>
                    <List.Item.Meta avatar={<Avatar>{initialFor(who)}</Avatar>} title={who} description={<Tag color={perm.color}>{perm.label}</Tag>} />
                  </List.Item>
                );
              }}
            />
