/** Human-readable document status for grids and banners. */
export function displayStatus(doc) {
  if (!doc) return { label: "—", color: "default" };
  if (doc.state === "draft") return { label: "Draft — submit for review", color: "gold" };
  if (doc.state === "under_revision") return { label: "Under revision", color: "volcano" };
  if (doc.state === "changes_requested") return { label: "Sent back — fix & resubmit", color: "orange" };
  if (doc.state === "approved") return { label: "Approved", color: "green" };
  if (doc.state === "archived") return { label: "Archived", color: "default" };
  if (doc.state === "in_review") {
    if (doc.workflow_stage === "approval") return { label: "Reviewed — with approvers", color: "purple" };
    if (doc.workflow_stage === "review") return { label: "With reviewers", color: "blue" };
    return { label: "In review", color: "blue" };
  }
