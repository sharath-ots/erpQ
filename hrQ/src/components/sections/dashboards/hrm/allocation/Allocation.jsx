...
import { months } from "../../../../../data/common/months.js";
    ...
    <Paper sx={{ p: { xs: 3, md: 5 } }}>
      ...
      <Box
        sx={{
          ...
          gap: 5,
          overflow: "hidden",
          gridTemplateColumns: { xs: "1fr", sm: "1fr 240px", lg: "1fr", xl: "1fr 240px" },
        }}
      >
        <Box sx={{ minWidth: 0 }}>
          <Stack direction="column" sx={{ gap: 3 }}>
            <Stack sx={{ gap: 2, alignItems: "center" }}>
              {chartLegends.map(({ label, color }) => (
                <ChartLegend key={label} state={legendState} label={label} color={color} onToggle={handleLegendToggle} />
              ))}
            </Stack>
            <Box sx={{ minWidth: 0 }}>
