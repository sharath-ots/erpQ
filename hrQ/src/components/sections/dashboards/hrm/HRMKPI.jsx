...
const getChipStyle = (value) => {
  if (value > 0) return { icon: "material-symbols:trending-up-rounded", color: "success" };
  if (value < 0) return { icon: "material-symbols:trending-down-rounded", color: "error" };
  return { icon: "material-symbols:trending-flat-rounded", color: "neutral" };
};
  ...
  return (
    <Paper component={Stack} direction="column" sx={{ p: { xs: 3, md: 5 }, pt: { xs: 2, md: 4 }, height: 1 }}>
      <Stack
        ...
        sx={{ mb: { xs: 2, xl: 1 }, gap: 1, alignItems: { xs: "flex-start", sm: "baseline", md: "flex-start", xl: "baseline" } }}
      >
        <Typography variant="h5" sx={{ fontWeight: 500 }}>
          {typeof value === "number" ? numberFormat(value) : value}
        </Typography>
        <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>{subtitle}</Typography>
      </Stack>
      ...
      >
        ...
        />
        <Typography variant="subtitle2" sx={{ color: "text.secondary" }}>{`Since ${change.since}`}</Typography>
      </Stack>
    </Paper>
  );
};
