  ...
  return (
    ...
    >
      <Stack sx={{ width: 160, alignItems: "center", justifyContent: "space-between" }}>
        <Button shape="square" color="neutral" onClick={() => calendarRef.current?.goToPreviousMonth()}>
          <IconifyIcon flipOnRTL icon="material-symbols:chevron-left-rounded" sx={{ fontSize: 20 }} />
        </Button>
        <Typography variant="body2" sx={{ color: "text.secondary", textWrap: "nowrap", textAlign: "center", minWidth: 75 }}>
          {dayjs(currentDate).format("MMM, YYYY")}
        </Typography>
        <Button shape="square" color="neutral" onClick={() => calendarRef.current?.goToNextMonth()}>
          <IconifyIcon flipOnRTL icon="material-symbols:chevron-right-rounded" sx={{ fontSize: 20 }} />
        </Button>
      </Stack>
      ...
      <StyledTextField
        ...
        sx={{
          ...
          [`& .${selectClasses.select}`]: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1 },
        }}
        select
      >
        {users.map((user) => (
          <MenuItem key={user.name} value={user.name} sx={{ gap: 1 }}>
            ...
            <Typography variant="body2" sx={{ textOverflow: "ellipsis", overflow: "hidden", whiteSpace: "nowrap", display: "block", width: 1 }}>
              {user.name}
            </Typography>
          </MenuItem>
        ))}
